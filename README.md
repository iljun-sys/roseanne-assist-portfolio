...
<img src="screenshots/staff-02-briefing-panel.png" alt="스태프 환자 브리핑" />

- 문진표 원본을 진료용으로 재구성, **EMR 복사**(병원 EMR에 붙여넣을 형식)

### 원장 — 진료실

<img src="screenshots/doctor-01-workspace.png" alt="원장 진료실 — 배정 환자·진료 메모·문진표 원본·SOAP 요약" />

- 한 화면에 **배정된 환자 · 진료 메모 · 문진표 원본 · SOAP 요약** — 왼쪽에서 환자를 고르면 나머지가 바뀜
- **SOAP 요약**은 문진에 없는 사실을 쓰지 않도록 규칙을 두고, 「AI 생성 · 참고용」과 생성 모델을 함께 표시
- 환자별 **진료 메모**(자동 저장)와 **음성 녹음** — 녹음은 전사 후 요약해 기록 부담을 줄임
- **즉시 확인 필요**(복용약·기저질환)를 맨 위에 두고, 갱년기 환자는 **KI 점수**와 상위 증상을 함께 표시
- **AI 브리핑**(증상 요약·감별진단 힌트·추가 문진 추천)도 문진표 원본 옆에 두어 원자료와 대조하며 봄

#### 환자 안내문

<img src="screenshots/doctor-02-handout.png" alt="환자 안내문 — AI 초안 편집과 인쇄 미리보기" />

- 진료 메모와 녹음 요약으로 **AI가 쉬운 말 초안**을 만들고, 의사가 왼쪽에서 고치면 오른쪽 인쇄본에 바로 반영
- AI 출력은 저장하지 않고 **의사가 확인한 확정본만 인쇄** — 입력에 없는 진단·약·수치는 쓰지 않도록 제한

#### 전체 현황

<img src="screenshots/doctor-03-overview.png" alt="전체 현황 — 원장별 환자 배치" />

- 다른 원장에게 배정된 환자와 미배정 환자까지 한눈에, 원장별 대기·진료·완료 수를 머리에 표시
...
**7. AI 환각의 원인을 입력 데이터에서 찾기**
SOAP 요약에 환자가 답하지 않은 "Pap smear 정상"과 하루 밀린 생리일이 나왔습니다. 원인은 프롬프트보다 입력에 있었습니다 — 예진 화면이 날짜 칸에 오늘 날짜를 미리 넣고 「검사 이력 있음」을 기본으로 눌러 두어 건너뛴 문항도 '있음'으로 저장됐고, 날짜는 UTC 변환으로 하루 앞당겨졌습니다. 기본값을 없애고 읽기 모델에서 과거 데이터까지 정리한 뒤, 프롬프트에 사실 규칙과 temperature 0.2를 더하고 운영 AI로 결과를 확인했습니다.
